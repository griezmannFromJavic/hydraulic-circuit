(define (lagrange-interpolation points)	; returns function of x
	...
	(define (list-of-factors point) ; returns list of functions of x
		(map (lambda (p) (basis-factor point p)) (remove (lambda (p) (equal? point p)) points)) ; TU JE BUG! map mapira jednu funkciju na cijelu listu
		)
	(define (basis-polynomial point)
		(multiply-list-of-functions (list-of-factors point))
		)
	(multiply-list-of-functions (map multiply-function-with-number (map cadr points) (map basis-polynomial points)))
	)
